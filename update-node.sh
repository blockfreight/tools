...
do
   ...
   ssh -oStrictHostKeyChecking=no $BLOCKFREIGHT_SSH_USER@$validator "env validator_name=$validator_name;curl https://raw.githubusercontent.com/blockfreight/tools/master/blockfreightnet-kubernetes/examples/blockfreight/app.yaml > app.yaml;sed -e 's/<VALIDATOR_NAME>/$validator_name/g; s/<PRIVATE_NODE_KEY>/$private_node_key/g; s/<PRIVATE_KEY>/$private_key/g; s/<PRIVATE_TYPE>/$private_type/g' -i app.yaml;cat app.yaml;kubectl apply -f app.yaml && kubectl delete pods --all --grace-period=0 --force;" <<-'ENDSSH'
